...
func main() {
	run := func() int {
		...
		ctx, log := m.StartRootSpan(ctx, "spin")
		defer func() {
...
func buildRootCommand(m *spin.Mixin, in io.Reader) *cobra.Command {
	cmd := &cobra.Command{
		Use:  "Spin",
		Long: "A spin mixin to use for building other mixins for porter 👩🏽‍✈️",
